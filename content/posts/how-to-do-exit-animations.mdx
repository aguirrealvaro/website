...
description: My take on adding animations when a react component unmounts. Without any animation library. Example with a Modal component
...
Something that is worth to mention, we used 150ms for the transition between each phase for useDisclosure hook, and we also used the same value for the animation-time for the animations inside Dialog. Ideally we save this value inside a variable to make sure it is the same in all the places.
